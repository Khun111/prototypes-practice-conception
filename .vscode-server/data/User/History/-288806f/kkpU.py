...
def main():
    ...
    # Create a dictionary to store the database data and set STR counts to the longest run of each using the helper function
    longest_run = {}
    data = databases[0].keys()
    for i in data:
...
# from the matches list, select the most possible match from the person with highest matches in the list, else print no match
    none = "No match"
    count = Counter(matches)
    for i in count.keys():
        if (count[i]) == len(data) - 1:
            none = i
            break
    print(none)
    return
